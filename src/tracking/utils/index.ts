...
export const calculateChangeBetweenNumber = (
  calculateNumber: BigNumber,
  min: BigNumber,
  max: BigNumber,
  range = 10
): string => {
  ...
  if (remainderNumber.length >= 2) {
    remainderNumber =
      remainderNumber.slice(0, 1) +
      '.' +
      remainderNumber.slice(1, remainderNumber.length)
  }
...
export const calculateChangeNumber = (
