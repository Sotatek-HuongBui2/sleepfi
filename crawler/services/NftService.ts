...
import {getAdminPrv, getNFTAdminPrv, NFT_CONTRACT, NFT_TYPE_SUPPORT} from "../common/Utils";
...
export default class NftService {
    ...
    async handleMintNft(number, nftType) {
        ...
        const admin = process.env.ADMIN_MAIN_WALLET
        const nft = new web3.eth.Contract(ERC721, contract);
        const nftPrvk = await getAdminPrv()
        if (!nftPrvk) return
