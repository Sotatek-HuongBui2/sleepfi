...
export default class JewelMintCrawler {
  public abi: any
  ...
  constructor() {
    this.contract = process.env.JEWEL_CONTRACT
    this.abi = ERC721
    this.startBlock = Number(process.env.JEWEL_CONTRACT_START_BLOCK)
  ...
  async crawlBlock(fromBlock: number, toBlock: number) {
    try {
      ...
      for (const event of events) {
        await NftService.getInstance().handleEventMintNft(event, "jewel")
      }
